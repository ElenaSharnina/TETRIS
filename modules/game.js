...
export class Game {
  ...
  moveDown() {
    ...
    if (
      ...
      this.stopMove();
      this.sound("../../music/bullethits.mp3");
    }
  }
  rotateTetromino() {
  ...
  visibleGameOver() {
    if (this.gameOver) {
      this.sound("../../music/08-game-over.mp3");
      setTimeout( () => {
      ...
      <p class="game-over__text">For the continie, press SPACE</p>
      <audio src="./music/Мелодия из тетриса - тетрис.mp3" autoplay="autoplay" class="audio"></audio>
    </div>`
      );
  ...
  clearRow() {
    ...
    for (let i = ROWS - 1; i >= 0; i--) {
      ...
      if (countBlock === COLUMNS) {
        rows.unshift(i);
        this.sound("../../music/07-stage-clear.mp3");
      }
    }
    rows.forEach((i) => {
